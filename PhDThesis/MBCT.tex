...
%The first step in any in-silico pre-clinical trial is to choose a patient model that can interact with the device in a closed loop. 
...
The node and path topology used in the in-silico pre-clinical trials is shown in Fig. \ref{fig:HM_top}. 
...
In this chapter we implemented the detection algorithm Rhythm ID of Boston Scientific (\cite{compass,Ellenbogen11_Pacingbook}),% [???Compass, pacingdefibrillation,VTC paper, ICD book, Ellenbogen].
...
\begin{figure}[t]
	\centering
	\includegraphics[scale=0.4]{figures/figScreenshot.pdf}
...
This illustrates very well the benefit that an in-silico pre-clinical trial can bring to the planning of an RCT: the fact that Rhythm ID could not be shown to be better than PRL+W %(let alone 25\% better as was hypothesized by the investigators) 
...
%\textbf{Sample size calculation for in-silico pre-clinical trial}.
...
%We used 11400 instances in our in-silico pre-clinical trial.
...
We calculated sensitivity and specificity in our in-silico pre-clinical trials, and report them in \tableref{vtsvt} on a per-arrhythmia basis.
...
%Each in-silico pre-clinical trial took approximately 3 hours on a 3GHz Intel Xeon processor with 8 cores. The resulting specificity values are shown in Fig. \ref{fig:parameter}. 
...
%However, it does point the way to an interesting application of in-silico pre-clinical trial, namely studying the sensitivity of a trial's results to parameter setting strategy.
...
It is important to stress that in general, one should not expect \emph{absolute numbers} from an in-silico pre-clinical trial to match those from a clinical trial, nor should this be the goal.
